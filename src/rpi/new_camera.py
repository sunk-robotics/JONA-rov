...
class StreamingOutput(io.BufferedIOBase):
    def __init__(self, new_frame_event):
        self.frame = None
...
class WSServer:
    ...
    output = StreamingOutput(new_frame_event)
    clients = set()
    ...
    async def listen_for_frame(cls):
        while True:
            if cls.output.frame is None:
                ...
                continue
            broadcast(cls.clients, bytearray(cls.output.frame))
            # for websocket in cls.clients:
            #     try:
